...
template <typename T> pilha<T>::pilha(const pilha& outra) {
    N = outra.N;
    topo = outra.topo; //Copia N, topo da "outra" pilha;

    buffer = new T[N]; //Cria um buffer e colaca os dados em suas mesmas posições.
    for(int cont = 0; cont < topo; cont++) buffer[cont] = outra.buffer[cont];
}
